...
-- Ejercicio básicos
-- 1. El departamento de recursos humanos desea que una consulta muestre el
-- apellido, la identificación del trabajo, la fecha de contratación y la
-- identificación del empleado para cada empleado, apareciendo primero la
-- identificación del empleado. Proporcione un alias STARTDATE para la
-- columna HIRE_DATE.
SELECT employee_id, last_name, job_id, hire_date as startdate  FROM employees

-- 2. El departamento de recursos humanos desea que una consulta muestre
-- todos los ID de trabajo únicos de la tabla EMPLOYEES.
SELECT DISTINCT job_id FROM employees

-- Ejercicios avanzados
-- 3. El departamento de recursos humanos quiere encabezados de columna más
-- descriptivos para su informe sobre los empleados (consulta 1.1). Nombre los
-- encabezados de columna Emp #, Employee, Job y Hire Date,
-- respectivamente.
SELECT employee_id as "Emp #", last_name || ' ' || first_name "Employee", job_id "Job", hire_date "Hire Date"  FROM employees

-- 4. El departamento de recursos humanos ha solicitado un informe de todos los
-- empleados y sus ID de trabajo. Muestre el apellido concatenado con la ID del
-- trabajo (separados por una coma y un espacio) y nombre la columna
-- Employee and Title.
SELECT last_name || ',' || job_id AS "Employee and Title" FROM employees 

-- 5. Para familiarizarse con los datos en la tabla EMPLOYEES, cree una consulta
-- para mostrar todos los datos de esa tabla. Separe cada columna de salida
-- por una coma. Nombre el título de la columna THE_OUTPUT.
...
-- empleados en los departamentos 11 o 9 en orden alfabético ascendente por
-- nombre.
SELECT last_name, department_id FROM employees
WHERE department_id = 11 OR department_id = 9
ORDER BY last_name ASC

-- 6. Crea un informe para mostrar el apellido y el salario de los empleados que
-- ganan entre $ 5.000 y $ 12.000, y están en el departamento 11 o 9. Etiquete
...
-- 11. Muestre los apellidos de todos los empleados que tienen tanto una "a" como
...
WHERE (job_id = 16 OR job_id = 18) AND SALARY NOT IN (2500, 3500, 7000)
