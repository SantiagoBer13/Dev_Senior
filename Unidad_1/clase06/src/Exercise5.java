...
public class Exercise5 {
    ...
    public static void showFailFrecuently() {
        ...
        failesFrecuentes.forEach((lote, fallosPorTipo) -> {
            ...
            fallosPorTipo.forEach((tipoFallo, cantidad) -> System.out.printf("  Tipo de fallo: %s, cantidad: %d%n",
                    tipoFallo, cantidad));
        });
    }
    ...
    public static void getLoteMayorFallos() {
        ...
        Map<String, Map<String, Integer>> lotCountInci = fallos.stream()
                ...
                .collect(Collectors.groupingBy(
                        ...
                        Collectors.groupingBy(
                                fallo -> fallo[1],
                                Collectors.summingInt(fallo -> Integer.parseInt(fallo[2].trim())))));
        ...
        Map.Entry<String, Map<String, Integer>> loteMayor = lotCountInci.entrySet().stream()
                .max(Comparator
                        .comparingInt(entry -> entry.getValue().values().stream().mapToInt(Integer::intValue).sum()))
                .orElse(null);
        ...
        fallosPorLote
                .forEach((fallo, cantidad) -> System.out.printf("    Fallo: %s, Cantidad: %d%n", fallo, cantidad));
    ...
    public static void calPromTypeInspection() {
        ...
        Map<String, Double> promTypeInspection = inspecciones.stream().map(row -> row.split(","))
                .collect(Collectors.groupingBy(
                        ...
                        Collectors.averagingDouble(
                                inspection -> Double.parseDouble(inspection[2].trim()))));
        ...
        if (inspeccionConPromedioMasBajo != null) {
            ...
            System.out.println("puede necesitar ajustes en los criterios de prueba o procesos de inspección.");
        }
    }
    ...
    public static void ideLotFalCriticInspeccion() {
        ...
        lotFallCritic.forEach((lote, aprobacion) -> {
            ...
            if (fallosAsociados != null && !fallosAsociados.isEmpty()) {
                fallosAsociados.forEach((fallo, cantidad) -> System.out.printf("    Tipo de Fallo: %s.%n", fallo));
            } else {
        ...
        indiceAprobacionInspection = inspecciones.stream()
                ...
                .collect(Collectors.groupingBy(
                        ...
                        Collectors.groupingBy(
                                ...
                                Collectors.mapping(
                                        registro -> Double.parseDouble(registro[2].trim()),
                                        Collectors.toList() 
                                )
                        )
                ));
